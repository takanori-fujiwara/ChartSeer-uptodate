...
class ModelVAE():
    ...
    def create(self,
        ...
        self.decoder = Model(
            encoded_input,
            self._buildDecoder(encoded_input, latent_rep_size, self.max_length,
                               self.rules_length))
        ...
        self.autoencoder = Model(
            x1,
            self._buildDecoder(z1, latent_rep_size, self.max_length,
                               self.rules_length))
    ...
    def _encoderMeanVar(self,
                        x,
                        latent_rep_size,
                        max_length,
                        epsilon_std=0.01):
        h = Convolution1D(self.hypers['conv1'][0],
                          self.hypers['conv1'][1],
                          activation='relu',
                          name='conv_1')(x)
        h = BatchNormalization(name='batch_1')(h)
        h = Convolution1D(self.hypers['conv2'][0],
                          self.hypers['conv2'][1],
                          activation='relu',
                          name='conv_2')(h)
        h = BatchNormalization(name='batch_2')(h)
        h = Convolution1D(self.hypers['conv3'][0],
                          self.hypers['conv3'][1],
                          activation='relu',
                          name='conv_3')(h)
    ...
    def _buildEncoder(self, x, latent_rep_size, max_length, epsilon_std=0.01):
        ...
        def conditional(x_true, x_pred):
            ...
            ix2 = tf.expand_dims(tf.gather(self.ind_of_ind_K, most_likely),
                                 1)  # index ind_of_ind with res
            ix2 = tf.cast(ix2, tf.int32)  # cast indices as ints
            M2 = tf.gather_nd(self.masks_K,
                              ix2)  # get slices of masks_K with indices
            M3 = tf.reshape(
                M2, [-1, max_length, self.rules_length])  # reshape them
            P2 = tf.multiply(K.exp(x_pred),
                             M3)  # apply them to the exp-predictions
            P2 = tf.div(P2, K.sum(P2, axis=-1,
                                  keepdims=True))  # normalize predictions
            return P2
    ...
    def _buildDecoder(self, z, latent_rep_size, max_length, rules_length):
        ...
        return TimeDistributed(Dense(rules_length, activation='sigmoid'),
                               name='decoded_mean')(h)
