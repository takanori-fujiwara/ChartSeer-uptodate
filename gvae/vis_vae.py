...
class VisVAE():
    ...
    def __init__(self, weights_file, rules, max_len, latent_dim):
        ...
        self.vae = ModelVAE()
        self.vae.load(self.rules,
                      weights_file,
                      max_length=self.max_len,
                      latent_rep_size=self.latent_dim,
                      hypers=hypers)

    def encode(self, sentences):
        one_hot = np.zeros((len(sentences), self.max_len, self.input_dim),
                           dtype=np.float32)
    ...
    def decode(self, z):
        ...
        prod_seq = [[
            self.productions[X_hat[index, t].argmax()]
    ...
    def _sample_using_masks(self, unmasked):
        ...
        for t in range(unmasked.shape[1]):
            next_nonterminal = [
                self.lhs_map[self._pop_or_nothing(a)] for a in S
            ]
            ...
            rhs = [
                filter(
                    ...
                    (str(a) != 'None'), self.productions[i].rhs())
